...
// setting up route endpoint for the home page
...
app.get('/movie/:movieId', (req, res) => {
  ...
  for (var j = 0; j < movies.length; j++) {
    if (movies[j].id === rMId) {
      ...
      return res.render('pages/movie', data);
    }
...
// setting up route endpoint for the search page

app.get('/search', (req, res) => {

  let searchCriteria = req.query.searchFor;

  let movies = getmovies();
  var array = [];
  for (var j = 0; j < movies.length; j++) {
    if  ( searchCriteria=== movies[j].year ) {
      array.push(movies[j]);
    }
  }
  var data = {
